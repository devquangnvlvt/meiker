...
def analyze_structure(raw_data):
    ...
    - static_items   : cac layer luon hien thi
    - id_to_path     : { image_node_id -> local_path tuong doi }
    - all_downloads  : [ { cdn_url, local_path } ]
    """
    manifest_cats = []
    static_items  = []
    id_to_path    = {}       # image id -> relative local path
    all_downloads = []       # list cac file can tai
    ...
    for node in raw_data:
        ...
        # Bo qua static layers hoan toan theo yeu cau cua user
        if ntype in ('image', 'simple-layer', 'image-dynamic') and ('static' in tags or 'static' in ntype):
            continue

        # ----- Category / Stamps (nguoi dung chon 1 trong nhieu option) ------
        if ntype in ('category', 'stamps', 'icon-set'):
            ...
            nav_x_counter += 1
            x_val = nav_x_counter
            y_val = canvas_y_counter

            # Đặt tên folder: X-Y (hoặc X-Y-Name nếu có tên thật)
            ...
            # Duyet cac OPTION (child truc tiep cua category)
            ...
            for opt_idx, child in enumerate(children, start=1):
                ...
                # Thu thap tat ca IMAGE node ben trong option nay
                ...
                if layers_info:
                    cat_entry['options'].append({
                        ...
                        'label':      opt_label,
                        'folder':     opt_folder,
                        'thumbnail':  layers_info[0]['local_path'],  # anh dau = thumbnail
                        'layers':     layers_info,
                        'visible':    child.get('visible', False),
                    })

            manifest_cats.append(cat_entry)
            continue

    return manifest_cats, id_to_path, all_downloads

# ─────────────────────────────────────────────────────────────────────────────
# Embed moi layer vao canvas canvas_w x canvas_h dung toa do x/y
# ─────────────────────────────────────────────────────────────────────────────

def embed_layers(manifest_cats, save_dir, cw, ch):
        ...
        for opt in cat['options']:
            for layer in opt['layers']:
                ...
                try:
                    ...
                    # Da la kich thuoc canvas -> bo qua
                    if sw == cw and sh == ch:
                        skip += 1; total += 1
                        continue
...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  #canvas-wrap { position: relative; background: #fff; box-shadow: 0 8px 32px rgba(0,0,0,.5); border-radius: 4px; overflow: hidden; transform-origin: top center; }
...
function renderCanvas() {
  ...
  manifest.categories.forEach((cat, ci) => {
    const optIdx = selected[ci];
    if (optIdx === undefined || optIdx === null) return;
    cat.options[optIdx].layers.forEach(layer => {
      const img = document.createElement('img');
      img.className = 'layer'; img.src = layer.local_path; img.draggable = false;
...
function saveImage() {
  ...
  manifest.categories.forEach((cat, ci) => {
    ...
    cat.options[oi].layers.forEach(l => layers.push(l));
  });
  if (!layers.length) { alert('Chua chon gi ca!'); return; }
  let loaded = 0;
  const imgs = layers.map(l => { const i = new Image(); i.src = l.local_path + '?' + Date.now(); return i; });
  imgs.forEach(img => {
...
def scrape_game(game_id):
    ...
    manifest_cats, id_to_path, all_downloads = analyze_structure(raw_data)
    print(f"    -> {len(manifest_cats)} categories | {len(all_downloads)} anh can tai")
    ...
    print(f"[+] Da luu manifest.json (chua toan bo thong tin toa do X/Y)")
    ...
    embed_layers(manifest_cats, save_dir, cw, ch)
